...
def get_embedding_for_speaker():
    ...
    for key in enrol_dict.keys():
        ...
        if spk_id in embeddings:
            embeddings[spk_id].append(enrol_dict[key])
        else:
            embeddings[spk_id] = []
            embeddings[spk_id].append(enrol_dict[key])
    for key in embeddings.keys():
        embeddings_for_spk[key] = sum(embeddings[key])/len(embeddings[key])
    return embeddings_for_spk    
...
if __name__ == "__main__":
    ...
    #)
    #veri_file_path = '/opt/speechbrain/recipes/VoxCeleb/SpeakerRec/results/voxceleb1_2/speaker_verification_ecapa_big/save/veri_test.txt'
    veri_file_path = './results/voxceleb1_2/speaker_verification_ecapa_big/save/veri_test.txt'
    #veri_file_path = 'veri_test.txt' 
    ...
    enrol_dict_spk = get_embedding_for_speaker()
    ...
    #veri_file_spk_path = '/opt/speechbrain/recipes/VoxCeleb/SpeakerRec/results/voxceleb1_2/speaker_verification_ecapa_big/save/veri_test2_spk.txt'
    veri_file_spk_path = './results/voxceleb1_2/speaker_verification_ecapa_big/save/veri_test2_spk.txt'
    with open(veri_file_spk_path) as f:
        veri_test = [line.rstrip() for line in f]

    positive_scores, negative_scores = get_verification_scores(veri_test)
    del enrol_dict, test_dict, enrol_dict_spk
