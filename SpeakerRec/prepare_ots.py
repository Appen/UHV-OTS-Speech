...
def prepare_ots_test(
    ...
    if "dev" in splits:
        prepare_csv(
            seg_dur, wav_lst_dev, save_csv_train, random_segment, amp_th
        )

    if "train" in splits:
        prepare_csv(seg_dur, wav_lst_train, save_csv_dev, random_segment, amp_th)
...
def _get_chunks(seg_dur, audio_id, audio_duration):
    ...
    """
    num_chunks = int(audio_duration * 1000 / seg_dur)  # all in milliseconds

    chunk_lst = [
        audio_id + "_" + str(i * seg_dur / 1000) + "_" + str((i * seg_dur + seg_dur)/1000)
